...
def vert_prof_eval(var,z_coor,z_or_z_range,interp_method='linear',extrap='NaN',
    ...
    NOTE: requires positive, monotonically-increasing vector of depths. 
...
def make_monotonic_with_nans(array):
    "input must be np.array not list"
    mask_not_monotonic = np.diff(array) < 0
    # Identify positions with NaNs
    mask_nans = np.isnan(array[:-1]) | np.isnan(array[1:])
    # Combine the two masks to preserve NaNs
    mask_replace = np.logical_and(mask_not_monotonic, ~mask_nans)
    # Replace non-monotonically increasing values with np.nan
    array[:-1][mask_replace] = np.nan

    return array


def calc_mld(sigma_theta, depth, ref_depth=10,ref_range_method='interp',ref_reject=False,sigma_theta_crit=0.03,crit_method='interp',
        crit_interp_initial_spacing=1.0,crit_interp_final_spacing=0.01,bottom_return='NaN',
        verbose_warn=False,verbose_error=True):
    ...
    # sigma_theta= data['sigma0'].values
    # print(sigma_theta)
    # print(sigma_theta.shape)
    # depth= data['depth'].values
    
    #need to add method to a) return np.nan if either depth and sigma0 are missing too much data, 
    #and b) make sure they are the same length. I think It makes sense to do that in the wrapper function 
    #in the main Float processsing script
    
    
    #check monotonically increasing depth
    depth_strp_nans=depth[~isnan(depth)]
    # print(depth_strp_nans)
    # print(depth_strp_nans.shape)
    if all(depth_strp_nans[i] <= depth_strp_nans[i + 1] for i in range(len(depth_strp_nans) - 1)) ==False:
        if verbose_warn:
            print("array is not montonically increasing")
        depth=make_monotonic_with_nans(depth)
    depth=depth[~isnan(sigma_theta)]
    sigma_theta=sigma_theta[~isnan(sigma_theta)]
    if verbose_warn:
        if any(isnan(sigma_theta.any()) or any(isnan(depth.any()))):
            print('Warning: sigma-theta or depth vector contains NaNs.')

        if ref_depth == 'shallowest':
            if depth[0] >= 20: print('Warning : shallowest measurement is 20 m or deeper.')
    ...
    if ref_depth == 'shallowest':
        ...
        rho_mld = sigma_theta_crit + vert_prof_eval(sigma_theta,depth,ref_depth,
    ...
    # print(rho_mld)
    mld_found = depth_at_which(sigma_theta,depth,rho_mld, method=crit_method,
