function [DATA,HEADERS,NICEHEADERS]=CO2SYSSOCCOM_smb(PAR1,PAR2,PAR1TYPE,PAR2TYPE,SAL,TEMPIN,TEMPOUT,PRESIN,PRESOUT,SI,PO4,pHSCALEIN,K1K2CONSTANTS,KSO4CONSTANTS);

% 2020_04_20 - global variables changed to passed variables.  Improves
% speed.  Tested with DIC and Alkalinity as inputs, but no other
% parameters.  Use with caution, ideally comparing initial output to
% CO2SYSSOCCOM.
...
% global  ; %fH RT
% global   ; %K0 K1 K2 KS KF  KW KB   KP1 KP2 KP3 KSi
% global ; %TS TF TB   TP TSi  F
...
[Pbar, TempK, logTempK, FugFac, VPFac, fH, RT, K0, K1, K2, TS, TF, KF, KS, KW, KB, KP1, KP2, KP3, KSi, TB] = Constants(TempCi,Pdbari, pHScale, WhichKs, WhoseKSO4, Sal, sqrSal, ntps, RGasConstant);
...
if any(F)
    [PHic(F) FCic(F)] = CalculatepHfCO2fromTATC(TAc(F)-PengCorrection(F), TCc(F), K0, K1, K2, TS, KS, TF, KF, KW, KB, KP1, KP2, KP3, KSi, TB, TP, TSi, F);
end
F=Icase==13; % input TA, pH
if any(F)
    TCc(F)  =   CalculateTCfromTApH(TAc(F)-PengCorrection(F), PHic(F), K1, K2, TS, KS, TF, KF, KW, KB, KP1, KP2, KP3, KSi, TB, TP, TSi, F);
    FCic(F) = CalculatefCO2fromTCpH(TCc(F), PHic(F), K0, K1, K2, F);
end
F=Icase==14 | Icase==15; % input TA, (pCO2 or fCO2)
if any(F)
    PHic(F) = CalculatepHfromTAfCO2(TAc(F)-PengCorrection(F), FCic(F), K0, K1, K2, TS, KS, TF, KF, KW, KB, KP1, KP2, KP3, KSi, TB, TP, TSi, F);
    TCc(F)  = CalculateTCfromTApH  (TAc(F)-PengCorrection(F), PHic(F), K1, K2, TS, KS, TF, KF, KW, KB, KP1, KP2, KP3, KSi, TB, TP, TSi, F);
end
F=Icase==23; % input TC, pH
if any(F)
    TAc(F)  = CalculateTAfromTCpH  (TCc(F), PHic(F), K1, K2, TS, KS, TF, KF, KW, KB, KP1, KP2, KP3, KSi, TB, TP, TSi, F) + PengCorrection(F);
    FCic(F) = CalculatefCO2fromTCpH(TCc(F), PHic(F), K0, K1 ,K2, F);
end
F=Icase==24 | Icase==25;  % input TC, (pCO2 or fCO2)
if any(F)
    PHic(F) = CalculatepHfromTCfCO2(TCc(F), FCic(F), K0, K1 ,K2, F);
    TAc(F)  = CalculateTAfromTCpH  (TCc(F), PHic(F), K1, K2, TS, KS, TF, KF, KW, KB, KP1, KP2, KP3, KSi, TB, TP, TSi, F) + PengCorrection(F);
end
F=Icase==34 | Icase==35; % input pH, (pCO2 or fCO2)
if any(F)
    TCc(F)  = CalculateTCfrompHfCO2(PHic(F), FCic(F), K0, K1, K2, F);
    TAc(F)  = CalculateTAfromTCpH  (TCc(F),  PHic(F), K1 ,K2, TS, KS, TF, KF, KW, KB, KP1, KP2, KP3, KSi, TB, TP, TSi, F) + PengCorrection(F);
end
...
[HCO3inp CO3inp BAlkinp...
    OHinp PAlkinp...
    SiAlkinp Hfreeinp...
    HSO4inp HFinp]      = CalculateAlkParts(PHic, TCc, K1, K2, TS, KS, TF, KF, KW, KB, KP1, KP2, KP3, KSi, TB, TP, TSi);
PAlkinp                 = PAlkinp+PengCorrection;
CO2inp                  = TCc - CO3inp - HCO3inp;
F=true(ntps,1);           % i.e., do for all samples:
Revelleinp              = RevelleFactor(TAc-PengCorrection, TCc, K0, K1, K2, TS, KS, TF, KF, KW, KB, KP1, KP2, KP3, KSi, TB, TP, TSi, F);
[OmegaCainp OmegaArinp] = CaSolubility(Sal, TempCi, Pdbari, TCc, PHic, WhichKs, Pbar, sqrSal, TempK, logTempK, ntps, RT, K1, K2);
xCO2dryinp              = PCic./VPFac; % ' this assumes pTot = 1 atm

% Just for reference, convert pH at input conditions to the other scales, too. 
[pHicT pHicS pHicF pHicN]=FindpHOnAllScales(PHic, pHScale, ntps, fH, TS, KS, TF, KF);
...
[Pbar, TempK, logTempK, FugFac, VPFac, fH, RT, K0, K1, K2, TS, TF, KF, KS, KW, KB, KP1, KP2, KP3, KSi, TB] = Constants(TempCo,Pdbaro,pHScale, WhichKs, WhoseKSO4, Sal, sqrSal, ntps, RGasConstant);

% Calculate, for output conditions, using conservative TA and TC, pH, fCO2 and pCO2
F=true(ntps,1); % i.e., do for all samples:
[PHoc FCoc] = CalculatepHfCO2fromTATC(TAc-PengCorrection, TCc, K0, K1, K2, TS, KS, TF, KF, KW, KB, KP1, KP2, KP3, KSi, TB, TP, TSi, F);
PCoc = FCoc./FugFac;
...
[HCO3out CO3out BAlkout...
    OHout PAlkout...
    SiAlkout Hfreeout...
    HSO4out HFout]      = CalculateAlkParts(PHoc, TCc, K1, K2, TS, KS, TF, KF, KW, KB, KP1, KP2, KP3, KSi, TB, TP, TSi);
PAlkout                 = PAlkout+PengCorrection;
CO2out                  = TCc - CO3out - HCO3out;
Revelleout              = RevelleFactor(TAc, TCc, K0, K1, K2, TS, KS, TF, KF, KW, KB, KP1, KP2, KP3, KSi, TB, TP, TSi, F);
[OmegaCaout OmegaArout] = CaSolubility(Sal, TempCo, Pdbaro, TCc, PHoc, WhichKs, Pbar, sqrSal, TempK, logTempK, ntps, RT, K1, K2);
xCO2dryout              = PCoc./VPFac; % ' this assumes pTot = 1 atm

% Just for reference, convert pH at output conditions to the other scales, too. 
[pHocT pHocS pHocF pHocN]=FindpHOnAllScales(PHoc, pHScale, ntps, fH, TS, KS, TF, KF);
...
% 0.98.
nanmean((1 + TS./KS)./(1 + TS./KS + TF./KF));
% 
% clear global  VPFac   %Sal Pdbari K2 ntps TS KP3  F   
% clear global  KB    T   % WhichKs pHScale  Pdbaro FugFac KS TSi
% clear global K        %WhoseKSO4 sqrSal TempCi PengCorrection KF KSi  TB
% % clear global        %TempCo RGasConstant fH K0 TF KP1 KW
% clear global      %Pbar TempK logTempK RT K1 KP2  TP 
...
function [Pbar, TempK, logTempK, FugFac, VPFac, fH, RT, K0, K1, K2, TS, TF, KF, KS, KW, KB, KP1, KP2, KP3, KSi, TB] = ...
    Constants(TempC,Pdbar,pHScale, WhichKs, WhoseKSO4, Sal, sqrSal, ntps, RGasConstant)
% global      ; %pHScale WhichKs WhoseKSO4 Pbar sqrSal RT
% global       ; %TempK logTempK  FugFac VPFac ntps fH K0
% global  ; %K1 K2 KF KS KW KB KP1 KP2 KP3 KSi
% global       ; %Sal RGasConstant TS TF TB TP TSi
...
function varargout=CalculatepHfCO2fromTATC(TAx, TCx, K0, K1 ,K2, TS, KS, TF, KF, KW, KB, KP1, KP2, KP3, KSi, TB, TP, TSi, F)
% global  ; %FugFac F
...
pHx   = CalculatepHfromTATC(TAx, TCx, K1, K2, TS, KS, TF, KF, KW, KB, KP1, KP2, KP3, KSi, TB, TP, TSi, F); % pH is returned on the scale requested in "pHscale" (see 'constants'...)
fCO2x = CalculatefCO2fromTCpH(TCx, pHx, K0, K1, K2, F);
varargout{1} = pHx;
varargout{2} = fCO2x;
end % end nested function


function varargout=CalculatepHfromTATC(TAx, TCx, K1, K2, TS, KS, TF, KF, KW, KB, KP1, KP2, KP3, KSi, TB, TP, TSi, F)
% global        ; %pHScale WhichKs WhoseKSO4 Pbar sqrSal RT
% global       ; %TempK logTempK FugFac VPFac fH K0 ntps
% global ; % K1 K2  KF KS  KW KB KP1 KP2 KP3 KSi
% global  ; %TF TS TB TP TSi F
%Outputs pH
...
function varargout=CalculatefCO2fromTCpH(TCx, pHx, K0, K1, K2, F)
% global    %K0 K1 K2 F
...
function varargout=CalculateTCfromTApH(TAx, pHx, K1, K2, TS, KS, TF, KF, KW, KB, KP1, KP2, KP3, KSi, TB, TP, TSi, F)
% global   ; %K0 K1 K2 KF KS  KW KB KP1 KP2 KP3 KSi
% global  % TF TS TB  TP TSi  F
K1F=K1(F);   K2F=K2(F);   KWF=KW(F);
KP1F=KP1(F); KP2F=KP2(F); KP3F=KP3(F); TPF=TP(F);
TSiF=TSi(F); KSiF=KSi(F); TBF=TB(F);   KBF=KB(F);
...
function varargout=CalculatepHfromTAfCO2(TAi, fCO2i, K0, K1, K2, TS, KS, TF, KF, KW, KB, KP1, KP2, KP3, KSi, TB, TP, TSi, F)
% global ; % K0  K1 K2  KF KS KW KB KP1 KP2 KP3 KSi
% global  %TF TS TB  TP TSi  F
...
function varargout=CalculateTAfromTCpH(TCi, pHi, K1, K2, TS, KS, TF, KF, KW, KB, KP1, KP2, KP3, KSi, TB, TP, TSi, F)
% global    ; %K0 K1 K2 KF KS  KW KB KP1 KP2 KP3 KSi
% global  %TF TS   TB TP TSi F
...
function varargout=CalculatepHfromTCfCO2(TCi, fCO2i, K0, K1, K2, F)
% global    ; %K0 K1 K2 F
...
function varargout=CalculateTCfrompHfCO2(pHi, fCO2i, K0, K1, K2, F)
% global    ; % K0 K1 K2 F
...
function varargout=RevelleFactor(TAi, TCi, K0, K1, K2, TS, KS, TF, KF, KW, KB, KP1, KP2, KP3, KSi, TB, TP, TSi, F)
% global WhichKs;
...
TCi = TC0 + dTC;
pHc= CalculatepHfromTATC(TAi, TCi, K1, K2, TS, KS, TF, KF, KW, KB, KP1, KP2, KP3, KSi, TB, TP, TSi, F);
fCO2c= CalculatefCO2fromTCpH(TCi, pHc, K0, K1, K2, F);
fCO2plus = fCO2c;
% ' Find fCO2 at TA, TC - dTC
TCi = TC0 - dTC;
pHc= CalculatepHfromTATC(TAi, TCi, K1 ,K2, TS, KS, TF, KF, KW, KB, KP1, KP2, KP3, KSi, TB, TP, TSi, F);
...
function varargout=CalculateAlkParts(pHx, TCx, K1, K2, TS, KS, TF, KF, KW, KB, KP1, KP2, KP3, KSi, TB, TP, TSi)
% global    ; %K0 K1 K2 KF KS KW KB KP1 KP2 KP3 KSi
% global F; %TF TS TB TP TSi 
...
function varargout=CaSolubility(Sal, TempC, Pdbar, TC, pH, WhichKs, Pbar, sqrSal, TempK, logTempK, ntps, RT, K1, K2)
% global         %WhichKs Pbar sqrSal TempK logTempK ntps RT K1 K2 
% '***********************************************************************
...
function varargout=FindpHOnAllScales(pH, pHScale, ntps, fH, TS, KS, TF, KF)
% global    ; %pHScale ntps fH K T
