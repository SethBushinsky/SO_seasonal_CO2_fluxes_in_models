...
%
function distance=taylor_dist_smb_for_manuscript_uncertainty(correlation, correlation_std, amplitude_ratio, amplitude_ratio_std,~, names, color_model, cmap, legend_on, rms_cutoff_for_good, out_of_phase_corr_cutoff, max_rel_amp)
...
corr_std_filt = correlation_std(~isnan(correlation));
ampl_std_filt = amplitude_ratio_std(~isnan(correlation));
...
% calculate sets of points for +/- std - need four total.
% first, plus/minus amplitude
theta=acos(corr_filt);    % compute correlation angle
rr = ampl_filt-ampl_std_filt; % amplitude minus 1 SD
xx_amp_low = rr.*cos(theta);
yy_amp_low=rr.*sin(theta);
clear rr theta

theta=acos(corr_filt);    % compute correlation angle
rr = ampl_filt+ampl_std_filt; % amplitude plus 1 SD
xx_amp_high = rr.*cos(theta);
yy_amp_high=rr.*sin(theta);

clear rr theta


% calculate sets of points for +/- std - need four total.
% second, plus/minus correlation
temporary_corr = corr_filt-corr_std_filt;
temporary_corr(temporary_corr>1)=1;
temporary_corr(temporary_corr<-1)=-1;
corr_low = temporary_corr;

temporary_corr = corr_filt+corr_std_filt;
temporary_corr(temporary_corr>1)=1;
temporary_corr(temporary_corr<-1)=-1;
corr_high = temporary_corr;

% loop through each model, calculate multiple points so that the
% correlation spread follows an arc:

n=20;
xx_corr = NaN(length(corr_filt), n+1);
yy_corr= NaN(length(corr_filt), n+1);

for m = 1:length(corr_filt)
    corr_range = corr_low(m):(corr_high(m)-corr_low(m))/n:corr_high(m);

    for cc = 1:length(corr_range)
        theta=acos(corr_range(cc));    % compute correlation angle
        rr = ampl_filt(m); % amplitude
        xx_corr(m,cc) = rr.*cos(theta);
        yy_corr(m,cc)=rr.*sin(theta);
        clear rr theta
    end
end

% theta=acos(temporary_corr);    % compute correlation angle
% rr = ampl_filt; % amplitude plus 1 SD
% xx_corr_high = rr.*cos(theta);
% yy_corr_high=rr.*sin(theta);
% 
% clear rr theta
...
% first plot error bars, then plot markers on top

for j=1:size(xx,1)

    marker_color = cmap(strmatch(names_filt{j}, color_model(:,1), 'exact'),:);

    plot([xx_amp_low(j) xx_amp_high(j)], [yy_amp_low(j) yy_amp_high(j)], '-', 'color', marker_color, 'linewidth', 2);
    % plot([xx_corr_low(j) xx_corr_high(j)], [yy_corr_low(j) yy_corr_high(j)], '-', 'color', marker_color, 'linewidth', 2);
    plot(xx_corr(j,:), yy_corr(j,:), '-', 'color', marker_color, 'linewidth', 2);
